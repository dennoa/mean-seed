...
describe('GitHub OAuth provider handler', function() {
  ...
  it('should retrieve the token from github', function(done) {
    var clientParams = {
      ...
      clientId: 'the github client id for the project',
    ...
    mockRequest.get = function(options, cb) {
      ...
      options.qs.code.should.equal(clientParams.code);
      options.qs.client_id.should.equal(clientParams.clientId);
      options.qs.redirect_uri.should.equal(clientParams.redirectUri);
    ...
    github(clientParams, function(err, userInfo) {
