...
describe('toUrl', function() {
  ...
  describe('when running behind a reverse-proxy', function() {
    beforeEach(module('meanSeed', function($provide) {
      $provide.value('$location', {
        absUrl: function() {
          return 'https://my-reverse-proxy.com.au/context/meanSeed/#/home';
        }
      });
    }));

    it('should convert a given path to a path that includes the full context', inject(function(toUrl) {
      expect(toUrl('/api/myservice')).toEqual('/context/meanSeed/api/myservice');
    }));
  });
  ...
  describe('when hitting the reverse-proxy directly', function() {
    beforeEach(module('meanSeed', function($provide) {
      $provide.value('$location', {
        absUrl: function() {
          return 'http://my-server:8080/meanSeed/#/home';
        }
      });
    }));

    it('should convert a given path to a path that includes the meanSeed app context', inject(function(toUrl) {
      expect(toUrl('/api/myservice')).toEqual('/meanSeed/api/myservice');
    }));
  });
  ...
  describe('when any other environment url has no hash', function() {
    beforeEach(module('meanSeed', function($provide) {
      $provide.value('$location', {
        absUrl: function() {
          return 'https://my-reverse-proxy.com.au/context/meanSeed/';
        }
      });
    }));
